...
var resizePizzas = function(size) {
    ...
    //gives back a sizing factor based on the desired size of the pie.
    ...
    function changePizzaSizes(size) {
        ...
        var pizzaOffsetWidth = randomPizzaContainers[0].offsetWidth; //always the same for all pizzas so just get it once!

        for (var i = 0; i < randomPizzaContainers.length; i++) {

            var dx = (sizeSwitcher(size) - (pizzaOffsetWidth / windowWidth)) * windowWidth;

            randomPizzaContainers[i].style.width = (pizzaOffsetWidth + dx) + 'px';
        }
    }
...
function updatePositions() {
    ...
    var scrollTopValue = document.body.scrollTop; //causes a layout and a repaint, we only need the value once.
